...
featureCounts -T 16 -g gene_id -J -G $genfilework -p -s 1 -d 20 -a $gtffile -o all.gene.counts.txt $(eval 'awk 'BEGIN { OFS = ""} {print $1,".bam"}' /home/put001/scripts2/SRA/SRA.txt | paste -sd " " -')
featureCounts -T 16 -g exon_id -f -J -G $genfilework -p -s 1 -d 20 -a $gtffile -o all.exon.counts.txt $(eval 'awk 'BEGIN { OFS = ""} {print $1,".bam"}' /home/put001/scripts2/SRA/SRA.txt | paste -sd " " -')
